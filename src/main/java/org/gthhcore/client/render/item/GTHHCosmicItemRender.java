package org.gthhcore.client.render.item;
...
public class GTHHCosmicItemRender extends WrappedItemRenderer {
    private static final HashMap<TextureAtlasSprite, IBakedModel> spriteQuadCache = new HashMap();
    private Random randy = new Random();

    public GTHHCosmicItemRender(IModelState state, IBakedModel model) {
        super(state, model);
    }

    public GTHHCosmicItemRender(IModelState state, WrappedItemRenderer.IWrappedModelGetter getter) {
        super(state, getter);
    }
    ...
    protected void renderSimple(ItemStack stack, EntityLivingBase player) {
        ...
        if (stack.getItem() instanceof ICosmicRenderItem) {
            ...
            IBakedModel cosmicModel = (IBakedModel)spriteQuadCache.computeIfAbsent(cosmicSprite, GTHHCosmicItemRender::computeModel);
    ...
    protected void renderInventory(ItemStack stack, EntityLivingBase player) {
        ...
        if (stack.getItem() instanceof ICosmicRenderItem) {
            ...
            IBakedModel cosmicModel = (IBakedModel)spriteQuadCache.computeIfAbsent(sprite, GTHHCosmicItemRender::computeModel);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            CosmicShaderHelper.cosmicOpacity = cri.getMaskOpacity(stack, player);
